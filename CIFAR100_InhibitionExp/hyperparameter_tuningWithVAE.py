...
class Hyperparametr:
    ...
    def objective(self,trial):
        ...
        params = {
             ...
             "plastic_model_update_freq":trial.suggest_float("plastic_model_update_freq", 0.3, 0.55, step=0.05),
          #  "total_epochs": trial.suggest_int("total_epochs",5,20,5),   #don't use num_epochs, it matches with some reserved names and throws error
            "reg_weight": trial.suggest_float("reg_weight", 0.65, 0.95, step=0.05),
        ...
        train_transformInput=train_transformInput,val_transformInput=val_transformInput,clipping=True) #CLS strategy


        ################################ 
        # Generator model
        #################################
        modelGR = VAE(channel_dim=img_channel_dim, latent_embedding = latent_embedding,batch_size=batch_sizeGR)
...
def main():
    ...
    study.optimize(StableAccuracyPerConfig, n_trials=16)
