...
class Hyperparametr:
    ...
    def dataGeneration(self):
        ...
        # train_indices, val_indices, _, _ = train_test_split(range(len(cifar_train)),cifar_train.targets,stratify=cifar_train.targets,
        # test_size=VAL_SIZE,)

        # # generate subset based on indices
        # train_split = Subset(cifar_train, train_indices)
        # val_split = Subset(cifar_train, val_indices)

        scenario_trainVal = nc_benchmark(cifar_train, cifar_test, n_experiences=5, shuffle=False, seed=None,
        fixed_class_order=[0,1,2,5,4,3,6,7,8,9],task_labels=False)
    ...
    def objective(self,trial):
        ...
        params = {
             "stable_model_update_freq": trial.suggest_float("stable_model_update_freq", 0.1, 0.3, step=0.05),
             "plastic_model_update_freq":trial.suggest_float("plastic_model_update_freq", 0.70, 0.95, step=0.05),
          #  "total_epochs": trial.suggest_int("total_epochs",5,20,5),   #don't use num_epochs, it matches with some reserved names and throws error
            "reg_weight": trial.suggest_float("reg_weight", 0.1, 0.35, step=0.05),
        ...
        # val_transformInput = Compose([transforms.ToPILImage(),ToTensor(),transforms.Normalize((0.4914, 0.4822, 0.4465),
        # (0.2470, 0.2435, 0.2615))])
        ...
        train_transformInput=train_transformInput,clipping=True) #CLS strategy
...
def main():
    ...
    study.optimize(StableAccuracyPerConfig, n_trials=13)
