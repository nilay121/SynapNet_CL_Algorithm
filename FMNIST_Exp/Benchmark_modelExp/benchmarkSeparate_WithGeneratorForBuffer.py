...
def main():
    ...
    num_syntheticExamplesPerDigit = 500#100#500
    ...
    train_transformJoint = Compose([transforms.ToTensor(), transforms.Normalize((0.2860,), (0.3530,))])
    ...
    fmnist_trainJoint = torchvision.datasets.FashionMNIST('./data/',train=True,download=True,transform=train_transformJoint) #only for joint training
    ...
    scenario_trainTest = nc_benchmark(fmnist_train, fmnist_test, n_experiences=n_experiences, shuffle=False, seed=9,task_labels=False)
    ...
    for counter in range(num_runs):
        ...
        siModel = SynapticIntelligence(model=modelsi,optimizer=SGD(modelsi.parameters(), lr=learning_rate, momentum=0.9),train_mb_size=train_batch_size,
        ...
        jointModel.train(fmnist_trainJoint)
...
def barPlotMeanPred(meanBenchEWC,meanBenchLWF,meanBenchSI,meanBenchNaive,meanBenchJoint,stdBenchEWC,stdBenchLWF,stdBenchSI,stdBenchNaive,stdBenchJoint,n_experinces):
    ...
    fig.tight_layout()
    plt.show()
    plt.savefig("FMNIST/buffer_size5000_std_test2.png")
