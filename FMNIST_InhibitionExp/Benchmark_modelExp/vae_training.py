...
class Vae_Cls_Generator:
  def __init__(self,num_epochs, model, device,learning_rate,batch_size,patience,img_channel_dim,reconstruction_term_weight=1):
    self.num_epochs = num_epochs
    self.device = device
    self.model =  model.to(self.device) 
    self.patience = patience
    self.img_channel_dim = img_channel_dim
    ...
    for epoch in range(self.num_epochs):
        ...
        for data in loader_loop:
          ...
          if len(buff_images) > 0:
            ...
            buffer_images = buffer_images.squeeze(2).reshape(-1,self.img_channel_dim,synthetic_imgHeight,synthetic_imgWidth).to(self.device)
