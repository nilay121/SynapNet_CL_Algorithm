...
class utility_funcs:
    def buffer_dataGeneration(digit, experience,device,model,num_examples,numbOf_orgExamples):
        ...
        idx = 0
        constraining_term = 1#0.15 #1 used in old cifar100 model
    ...
    def barPlotMeanPred(meanBenchEWC,meanBenchLWF,meanBenchSI,meanBenchNaive,meanBenchJoint,stdBenchEWC,stdBenchLWF,stdBenchSI,stdBenchNaive,stdBenchJoint,n_experinces):
        ...
        ## New changes
        ax.bar_label(bar_ewc, padding=3, rotation=90)
        ax.bar_label(bar_lwf, padding=3, rotation=90)
        ax.bar_label(bar_si, padding=3, rotation=90)
        ax.bar_label(bar_naive, padding=3, rotation=90)
        ax.bar_label(bar_joint, padding=3, rotation=90)
        y_min, y_max = ax.get_ylim()
        ax.set_ylim(y_min, y_max+0.1) 
        
        ax.set_title("CIFAR100 Benchmark", weight="bold")
        ...
        fig.tight_layout()
        plt.show()
        plt.savefig("CIFAR/cifar100_benchBold.png")
