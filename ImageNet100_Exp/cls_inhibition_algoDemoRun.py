...
class CustomInhibitStrategy():
  def __init__(self,working_model,modelstable,modelplastic,criterion_ce=torch.nn.CrossEntropyLoss(),
    ...
    self.working_model = working_model(self.n_classes).to(self.device) 
    self.modelstable = modelstable(self.n_classes).to(self.device) 
    self.modelplastic = modelplastic(self.n_classes).to(self.device)
  ...
  def train(self,experience,synthetic_imgHeight=28,synthetic_imgWidth=28,buf_inputs=[],buf_labels=[]):
    ...
    for epoch in range(self.num_epochs):
      ...
      if (epoch%self.gradMaskEpoch) == 0 and (epoch!=0) and (self.toDo_supression):   
        ## Conv1 masking
        gradient_conv1 = self.working_model.layer1[0].conv2.weight.grad
        ...
        if gradient_conv1 is not None:
            ...
            self.working_model.layer1[0].conv2.weight.grad = gradient_conv1.where(sup_mask_sized_as_x != 0, torch.zeros_like(gradient_conv1))
            print("#"*5,"Conv1 gradient updated with mask", "#"*5)
            ts.save(self.working_model.layer1[0].conv2.weight.grad, f"masks/Aftermask{experience.current_experience}.png")
