...
def main():
    ...
    #Train data transformations
    train_transformJoint = Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
    ...
    mnist_trainJoint = torchvision.datasets.MNIST('./data/',train=True,download=True,transform=train_transformJoint) #only for joint training
    ...
    scenario_trainTest = nc_benchmark(mnist_train, mnist_test, n_experiences=n_experiences, shuffle=False, 
    fixed_class_order= [1,2,3,4,5,6,7,8,9,0], task_labels=False)
    ...
    for counter in range(num_runs):
        ...
        jointModel.train(mnist_trainJoint)
