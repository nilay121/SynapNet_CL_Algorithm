...
class CustomInhibitStrategy():
  ...
  ## Training Phase
  def train(self, experience, synthetic_imgHeight=28,synthetic_imgWidth=28,buf_inputs=[],buf_labels=[], stable_model=None, plastic_model=None):
    if stable_model and plastic_model is not None:
      self.modelstable = stable_model
      self.modelplastic = plastic_model
    ...
    for epoch in range(self.num_epochs):
      ...
      for data,labels in loader_loop:
        input_dataBT = data.to(self.device) #Input data before transformations
        ...
        input_data = input_dataBT.reshape(-1,1,28,28)    
    ...
    return self.modelstable, self.modelplastic
  ...
  def offline_reorganizing(self,buf_inputs,buf_labels,patience=5,synthetic_imgHeight=28,synthetic_imgWidth=28,epochs=10,lr_offline=1e-5,offline_batch=32):
    ...
    for i in range(epochs):
      for j in range(batch_epochs):
        ...
        synthetic_imgHeight=synthetic_imgHeight,synthetic_imgWidth=synthetic_imgWidth,device=self.device,transform=self.train_transformBuffer)
        buffer_inputs = buffer_inputs.reshape(-1,1,28,28).to(self.device)
        buffer_labels = buffer_labels.to(self.device)
  ...
  ## Evaluation phase
  def evaluate(self,test_stream,exp_counter,validationFlag=False, stable_model=None, plastic_model=None):
    ...
    with torch.no_grad():
      ...
      for data, labels in tqdm(eval_data_loader):
        ...
        input_dataBT =  data.reshape(-1,1,28,28).to(self.device) 
        if validationFlag:
        ...
        tempStable.append(output_stable.cpu().detach().numpy())
        tempPlastic.append(output_plastic.cpu().detach().numpy())
      ...
      Plastic model accuracy : {accuracy_expPlastic/batch_counter}, working model accuracy : {accuracy_expWorking/batch_counter}")
      exp_counter+=1
    return accuracy_expStable/batch_counter,accuracy_expPlastic/batch_counter, tempStable, tempPlastic, predictionsForCF_stable, predictionsForCF_plastic